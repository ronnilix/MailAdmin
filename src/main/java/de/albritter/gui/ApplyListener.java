	...
	public void actionPerformed(ActionEvent e)
	{
		...
		case 0: // domain
			...
			else
			{
				...
				{
					JOptionPane.showMessageDialog(src, "Ihre Eingabe in dem Feld Domain ist ung\u00fcltig", "Fehler",
		...
		case 1: // mailbox
			...
			{
				if (src.getPanelMailbox().getPasswordField().getText()
						.equals(src.getPanelMailbox().getPasswordField_1().getText()))
				{
					((Mailbox) dataObject).setPassword(
							Sha512Crypt.Sha512_crypt(src.getPanelMailbox().getPasswordField().getText(), null, 5000));
				} else
				{
					JOptionPane.showMessageDialog(src, "Passw\u00f6rter stimmen nicht \u00fcberein", "Fehler",
							JOptionPane.ERROR_MESSAGE);
				}
			...
			} else
			{
				if (!e.getActionCommand().equals("Remove"))
				{
					JOptionPane.showMessageDialog(src, "Ihre Eingabe in dem Feld \"Mail Address\" ist ung\u00fcltig",
							"Fehler", JOptionPane.ERROR_MESSAGE);
					return;
				}
			}
			String notemailbox = src.getPanelMailbox().getTextareaNote().getText();
			((Mailbox) dataObject).setNote(notemailbox);
			...
			((Mailbox) dataObject)
					.setQuota(Integer.valueOf(String.valueOf(src.getPanelMailbox().getSpinnerQuota().getValue())));
			break;
		case 2: // Aliases
			...
			{
				JOptionPane.showMessageDialog(src, "Ihre Eingabe in dem Feld \"Source\" ist ung\u00fcltig", "Fehler",
						JOptionPane.ERROR_MESSAGE);
				if (!e.getActionCommand().equals("Remove"))
					return;
			}
			((Aliases) dataObject)
					.setSourceDomain((String) src.getPanelAliases().getComboBoxDomain().getSelectedItem());
			...
			} else
			{
				if (!e.getActionCommand().equals("Remove"))
				{
					JOptionPane.showMessageDialog(src, "Ihre Eingabe in dem Feld \"Destination\" ist ung\u00fcltig",
							"Fehler", JOptionPane.ERROR_MESSAGE);
					return;
				}
			}
			((Aliases) dataObject).setActive(src.getPanelAliases().getChckbxActive().isSelected() ? 1 : 0);
			String notealias = src.getPanelAliases().getTextareaNote().getText();
			((Aliases) dataObject).setNote(notealias);
			break;
		case 3: // TLS
